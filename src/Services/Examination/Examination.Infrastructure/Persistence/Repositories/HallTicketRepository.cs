...
internal sealed class HallTicketRepository : IHallTicketRepository
{
    private readonly ExaminationDbContext _context;
    public HallTicketRepository(ExaminationDbContext context, ITenantContext? tenant = null) => _context = context;
    ...
    public async Task<HallTicket?> GetByStudentExamAsync(Guid studentId, Guid examScheduleId, Guid tenantId, CancellationToken ct = default) =>
        await _context.HallTickets.FirstOrDefaultAsync(
            e => e.StudentId == studentId && e.ExamScheduleId == examScheduleId && e.TenantId == tenantId, ct);
